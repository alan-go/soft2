...
int main ( int argc, char **argv )
{
    ...
    if ( argc < 1 ) {
        cout << endl << "Usage: ./stereo_kitti path_to_vocabulary path_to_settings path_to_sequence" << endl;
        return 1;
    }
                  ...
                  vector<string> &vstrImageRight, vector<double> &vTimestamps )
{
    ...
    FILE *fp = fopen ( strPathTimeFile.c_str(),"r" );
    if ( !fp ) {
        return ;
    }
    while ( !feof ( fp ) ) {
...
for ( int i=0; i<nTimes; i++ )
{
	char num[10];
	sprintf(num,"%06d",i);
    vstrImageLeft[i] = strPrefixLeft + num + ".png";
    vstrImageRight[i] = strPrefixRight + num + ".png";
}
